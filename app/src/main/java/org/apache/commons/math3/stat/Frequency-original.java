...
/**
    ...
    public String[] getItems() {
        return freqTable.keySet().toArray(new String[getUniqueCount()]);
    }

    public ArrayList<WordItem> toWordsList() {
        Log.v("Frequency", "toWordsListSrart");

        ArrayList<WordItem> wordsList = new ArrayList<WordItem>() {
        };
        Iterator<Comparable<?>> iter = freqTable.keySet().iterator();
        while (iter.hasNext()) {

            Comparable<?> value = iter.next();
            WordItem w = new WordItem(value.toString(), getCount(value), getPct(value));
            wordsList.add(w);

        }
        Log.v("Frequency", "toWordsListFinish");
        Log.v("Frequency", "wordsList.size()" + wordsList.size());
        Log.v("Frequency", "getUniqueCount" + this.getUniqueCount());
